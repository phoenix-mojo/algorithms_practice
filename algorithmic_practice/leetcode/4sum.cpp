...
public:
    ...
    vector<vector<int> > twoSum(vector<int> nums, int target, int orig, int super, vector<vector<int> >&result) {
        ...
        for(int i = 0;i<nums.size();i++) {
            if(nums[i]<=target && target - nums[i] <= nums[nums.size()-1] && hash[target - nums[i]] != -1 && (i == nums.size()-1 || nums.size() == 2 || (i != nums.size()-1 && nums[i+1] != nums[i]))) {
            	...
            	temp.push_back(orig-target - 2*min);
    ...
    vector<vector<int> > threeSum(vector<int>& nums, int target, int orig, vector<vector<int> >&result) {
        for(int i = 0;i<nums.size()-2;i++) {    	
        	if(i == 0 || (i != 0 && nums[i] != nums[i-1])) {
    			vector<int> my_nums(nums.begin()+i+1, nums.end());
        		result = twoSum(my_nums, target-nums[i], target, orig, result);
        	}
        }
        return result;
    }
    ...
    vector<vector<int> > fourSum(vector<int>& nums, int target) {
        ...
        if(nums.size()<4)
            return result;
        front = nums[0];
        for(int i=0;nums.size()>=3;i++) {
            if(i==0 || (i!=0 && nums[i] != front)) {
                front = nums.front();
                nums.erase(nums.begin());
                result = threeSum(nums, target-front, front, result);
            }
        }
        return result;
    }
